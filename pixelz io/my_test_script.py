...
def my_put_packet(x,y, colour, proxies={}):
	packet = make_packet(x,y,colour)
	rx, ry = floor_to(x, floornum), floor_to(y, floornum)
	url = "http://pixelz.io/api/{}_{}/click".format(rx, ry)
	resp = requests.post(url, data=packet, proxies=proxies, timeout=5.)
...
# specific_packet = "\x02\x00\x25\x1f\x02"
# url = "http://pixelz.io/api/0_0/click"
# resp = requests.post(url, data=specific_packet, timeout=2.0)
# if resp.status_code != 200:
# 	raise RuntimeError(resp.text)
# print int(resp.text)

#print put_packet(33,33,2)

" ".join("{:02x}".format(ord(c)) for c in p)
p = "\x02\x00\xff"
